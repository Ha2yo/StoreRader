...
pub struct GoogleClaims {
    ...
    pub email: Option<String>,          // 유저 이메일
    pub email_verified: Option<bool>,   // 이메일 인증 여부
    pub exp: usize,                     // 만료 시각
    pub iat: Option<usize>,             // 발급 시각
    pub name: Option<String>,           // 유저 이름
    pub picture: Option<String>,        // 프로필 사진 URL
}
...
// 구글 ID Token 검증 함수
#[tauri::command]
pub async fn verify_google_id_token(id_token: String, client_id: String) -> Result<GoogleClaims, String> {
    ...
    let jwks: JwkResponse = resp.json().await.map_err(|e| format!("JWKS JSON 파싱 실패: {}", e))?;
